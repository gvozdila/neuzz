...
def process_data():
    ...
    print("MAX_BITMAP_SIZE, MAX_FILE_SIZE",MAX_BITMAP_SIZE, MAX_FILE_SIZE)
...
# compute jaccard accuracy for multiple label
# source https://torchmetrics.readthedocs.io/en/stable/classification/jaccard_index.html
def accur_1_pytorch(y_true, y_pred ):
    from torchmetrics.classification import MultilabelJaccardIndex
#    y_tr = torch.round(y_true)
#    pred = torch.round(y_pred)
#    summ = MAX_BITMAP_SIZE
#    metric = MultilabelJaccardIndex(num_labels=MAX_BITMAP_SIZE)
    
    # naive loss
    return torch.sum((y_pred - y_true) * (y_pred - y_true))
...
def compute_loss_grad(model,  loss_fn,  sample, target):
    """
    Function 
    ...
    @type pytorch loss func
    @param sample Single input - vectrized file 
    @type pytorch.tensor
    @param target Single target for model
    @type pytorch.tensor
    @return Torch autograd 
    @rtype    torch.autograd.grad for input sample
    """
    sample = torch.tensor(sample)
    sample = sample.unsqueeze(0).to(device)  # prepend batch dimension for processing
    sample.requires_grad == True
    normalized_input = Variable(sample, requires_grad=True).to(device)
    ...
    grad = normalized_input.grad
    print("loss=", loss , "sample=", normalized_input, "grad=", normalized_input.grad , "detach,cpu,sq",  normalized_input.detach().cpu().squeeze())
    # Do not need model grads, only grads w.r.t. input sample
    return loss.item(),  grad.cpu().detach().numpy()[0] # grad.cpu().detach().numpy() shape is (1,1,MAX_FILE_SIZE) we'll turn it in (1,MAX_FILE_SIZE)
...
def gen_adv2_pytorch(f, fl, model, loss_func, idxx, splice):
    """
           ...
           layer_list = (layer.name, layer)  from model.layers
            idxx = index of interested_indice
            1 = WTF?
    """
    adv_list = []
#    loss = layer_list[-2][1].output[:, f]
...
#        loss_value, grads_value = iterate([x])
        ...
        val = np.sign(grads_value[0][idx])
        adv_list.append((idx, val, fl[index]))
        #print(("idx=", idx,"val=",  val, "fl=", fl[index]))
    ...
    if splice == 1 and round_cnt != 0:
        if round_cnt % 2 == 0:
            ...
            val = np.sign(grads_value[0][idx])
            adv_list.append((idx, val, './splice_seeds/tmp_' + str(idxx + 500)))
    print(adv_list[0][0].shape, adv_list[0][1].shape)
    return adv_list
...
def gen_mutate2(model, edge_num, sign):
    ...
    with open('gradient_info_p', 'w') as f:
        for idxx in range(len(interested_indice[:])):
            # keras's would stall after multiple gradient compuation. Release memory and reload model to fix it.
            if idxx % 100 == 0:
                del model
                K.clear_session()
                model = build_model()
...
# grenerate gradient information to guide furture muatation
def gen_mutate2_pytorch(model, loss_func,  edge_num, sign):
    """
    Function 

    @param model NN model for grads calculation
    @type pytorch.model
    @param loss_func Loss function
    @type pytorch Loss
    @param edge_num Number of seeds to work with
    @type int
    @param sign Sign if currently in train mod (data[:5] == b"train")
    @type Boolean
    """
    tmp_list = []
    # select seeds
...
#                layer_list = [(layer.name, layer) for layer in model.layers]
            ...
            index = int(interested_indice[idxx])
            fl = [rand_seed1[idxx], rand_seed2[idxx]]
            adv_list = fn(index, fl, model, loss_func, idxx, 1)
            tmp_list.append(adv_list)
            for ele in adv_list:
                ele0 = [str(el) for el in ele[0]]
                ele1 = [str(int(el)) for el in ele[1]]
#                ele1 = [np.array2string((el.astype('int32')), threshold=None) for el in ele[1]]
                ele2 = ele[2]
                f.write(",".join(ele0) + '|' + ",".join(ele1) + '|' + ele2 + "\n")



def def_linear_model(input_dim, num_classes ):
    model = nn.Sequential(
    nn.Linear(in_features=input_dim, out_features=4096), 
...
def build_model():
    ...
    model = def_linear_model(input_dim=MAX_FILE_SIZE, num_classes=num_classes )
...
def fit(model, data,  optimizer,  criterion, epochs, lr , start_idx=1,  loss_history=[]):
    model.train()


    torch.cuda.empty_cache()
    print("Lets's fit model")
    # Losses & scores
...
#    # Save model and weights
    train_data = train_generate(64)
    fit(model=model, data = train_data,  optimizer=optimizer,  criterion = loss_func, epochs =2, lr = 0.001 , start_idx=1,  loss_history=loss_history)
    #model.save_weights("hard_label.h5")
    torch.save(model, "hard_label.h5")
...
def gen_grad(data):
    ...
    # model.load_weights('hard_label.h5')
    gen_mutate2_pytorch(model, loss_func, 500, data[:5] == b"train")
